...
function Superfeedr(login, password, resource) {
  ...
  this.client.on('stanza', function(stanza) {
    if(self.callbacks[stanza.attrs.id]) {
      ...
      var notification = self._parseNotification(stanza);
      if(notification) {
        if(notification.entries && notification.entries.length > 0) {
  ...
  this._parseSubscription = function _parseSubscription(subscription, url) {
    if (!subscription) {
      return null;
    }
  ...
  this._parseStatus = function _parseStatus(status, notification) {
    var httpStatus = status.getChild('http', 'http://superfeedr.com/xmpp-pubsub-ext');
    var nextFetch = status.getChild('next_fetch', 'http://superfeedr.com/xmpp-pubsub-ext');
    var title = status.getChild('title', 'http://superfeedr.com/xmpp-pubsub-ext');
    var period = status.getChild('period', 'http://superfeedr.com/xmpp-pubsub-ext');
  ...
  this._parseItems = function _parseItems(itemsNode) {
    var entries = [];
    if(itemsNode) {
  ...
  this._parseNotification = function _parseNotification(stanza) {
    var notification = {};
    ...
    notification = this._parseStatus(status, notification);

    var itemsNode = psevent.getChild('items', 'http://jabber.org/protocol/pubsub#event');
    notification.entries = this._parseItems(itemsNode);
...
Superfeedr.prototype.retrieve = function retrieve(url, clb) {
  var self = this;
  var id = Math.floor(Math.random()*100000).toString();
  var retrieveStanza = new xmpp.IQ({to: SUPERFEEDR_ENDPOINT, id:id, type:'get', from: self.client.jid})
  .c("pubsub", {xmlns: "http://jabber.org/protocol/pubsub"})
  .c("items", {node: url}).root();

  self.callbacks[id] = function(response, superfeedr) {
    if(response.attrs.type !== "result")
      return clb(new Error("Superfeedr responded with an error"), null);

    var pubsub = response.getChild('pubsub', 'http://jabber.org/protocol/pubsub');
    if(!pubsub)
      return clb(new Error(), null);

    var feed = {};
    feed.entries = [];

    var itemsNode = pubsub.getChild('items', 'http://jabber.org/protocol/pubsub');
    if(!itemsNode)
      return clb(null, feed);

    var feed = {};
    feed.entries = self._parseItems(itemsNode);

    return clb(null, feed);
  }
  self.client.send(retrieveStanza);
}
